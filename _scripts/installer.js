...
const copyFilesToVendorCssPath = (destFolder) => {
    ...
    if (!destFolder) {
        ...
        if ( outdatedFiles.length ) {
            console.info(
                '[stylutils] Removing outdated files:\n',
                outdatedFiles
            );
        ...
        console.info(
            '[stylutils] Updated files in "' + resolvedDestFolder + '".'
        );
    }
}
...
if (destFolderArg) {
    ...
    require('fs').writeFileSync(projectPath + 'package.json', JSON.stringify(projectPkg, null, 4));
